...
class TestSwitchBlocksOnConstructor(TestLocators):
    ...
    def test_navigate_to_fillings_block_from_buns_on_constructor_success(self, driver, login):
        ...
        driver.find_element(*self.fillings_block).click()
        assert driver.find_element(*self.selected_button).text == "Начинки"
    ...
    def test_navigate_to_fillings_block_from_sauces_on_constructor_success(self, driver):
        WebDriverWait(driver, 6).until(expected_conditions.visibility_of_element_located(self.button_login_in_main))
        driver.find_element(*self.sauces_block).click()
        driver.find_element(*self.fillings_block).click()
        assert driver.find_element(*self.selected_button).text == "Начинки"
    ...
    def test_navigate_to_sauces_block_from_buns_on_constructor_success(self, driver, login):
        WebDriverWait(driver, 6).until(expected_conditions.visibility_of_element_located(self.button_make_the_order))
        driver.find_element(*self.sauces_block).click()
        assert driver.find_element(*self.selected_button).text == "Соусы"
    ...
    def test_navigate_to_buns_block_from_sauces_on_constructor_success(self, driver, login):
        ...
        assert driver.find_element(*self.selected_button).text == "Булки"
